...
export const BESSCard: React.FC<BESSCardProps> = ({
  product,
  onViewDetails,
  onGetQuote,
}) => {
        ...
        <div className="p-3">
          ...
          <div className="mt-2 text-xs text-gray-300">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
              <span>Power: {product.powerRating || product.range}</span>
              <span>Capacity: {product.capacityRange}</span>
            </div>
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-1">
              <span>Chemistry: {product.chemistry || "LFP"}</span>
              <span>Voltage: {product.voltage}</span>
            </div>
