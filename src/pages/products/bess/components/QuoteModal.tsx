  ...
  useEffect(() => {
    if (product) {
      setQuoteForm({
        ...
        email: "",
        phone: "",
        message: `I'm interested in ${product?.name || "BESS"} (${product?.powerRating || product?.range || ""}).`,
      });
      setQuoteResult("");
    }
  }, [product]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Inquiring about: <strong>{product?.name || "BESS Solution"}</strong>. Provide your details and our team will contact you shortly.
          </DialogDescription>
        </DialogHeader>
        ...
        >
          <div>
            ...
            <input
              ...
              required
              className="w-full text-black px-3 py-2 rounded bg-white border border-gray-300 text-sm"
            />
          </div>
          <div>
            <label className="block text-sm mb-1">
